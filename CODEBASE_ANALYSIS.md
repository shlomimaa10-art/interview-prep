...
- **Excalidraw `groupIds`** — a precompute step builds `bandsByRow` (per row, contiguous same-zone bands) and mints a fresh `wbgz_*` groupId per band; each component maps to its band's id via `zoneGroupOfComp[i]` (or `null` when not in a band, e.g., hint-sized updates that skip backdrops). That id is stamped on the band's backdrop rect, zone label, every component shape + text label, and intra-band arrows + arrow labels — clicking any of them selects the whole band as one Excalidraw group. Cross-zone arrows stay ungrouped. The summary flow strip carries a single `wbgsum_*` groupId binding every dot, dot label, and connector arrow. `labelToMeta` carries `zoneGid` per component so the arrow emitter decides intra- vs cross-band membership without a re-lookup.
