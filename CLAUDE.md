...
2. **AI → Whiteboard (hint / full answer):** The AI can include a ` ```whiteboard ` JSON block at the end of its response. `renderWhiteboardUpdate(reply)` parses it and programmatically adds components and arrows to the Excalidraw canvas. Shape vocabulary: `ellipse` = client/user/external actor, `diamond` = decision/policy gate (rate-limit, auth, routing — **not** databases), `rectangle` = everything else (services, caches, queues, DBs; DB labels still get yellow auto-fill via `pickColor`). The renderer is zone-aware — `pickZone()` classifies each component into one of 7 semantic bands (client / edge / service / cache / async / data / observability) and `pickColor` defers to `ZONE_FILL[zone]`. On a full answer it additionally draws translucent dashed rounded-rect zone backdrops behind each contiguous same-zone band (IDs `wbz_*`), a horizontal ellipse/arrow summary flow strip above the diagram when ≥3 zones are present (IDs `wbsum_*`), and uses degree-based shape sizing (hero node deg ≥ 3 → 280×100, leaves deg ≤ 1 → 160×60, default 200×72). Arrow routing uses a Liang–Barsky segment-vs-rect check (plus multi-row span detection) to detour around intervening shapes via a 4-point orthogonal elbow path. Full-answer re-renders strip all prior generator IDs (`wbs_`/`wba_`/`wbal_`/`wbst_`/`wbz_`/`wbsum_`) so repeats replace cleanly; user-drawn elements (no prefix) are preserved. Generated text uses `fontFamily: 3` (Cascadia) to match the `roughness: 0` aesthetic. The ` ```whiteboard ` fence is stripped from the displayed chat text; an "✏️ *Updated whiteboard*" note is appended instead. The system prompt includes `WHITEBOARD_UPDATE FORMAT` instructions telling the AI when and how to emit these blocks (hint → single element; full answer → full diagram; feedback → text only). Emitted elements also carry Excalidraw `groupIds` so logical units select with one click: a precomputed `bandsByRow` table mints a fresh `wbgz_*` groupId per contiguous same-zone band (tracked per component in `zoneGroupOfComp[i]`, null when no band — e.g., hint updates that skip backdrops), and that id is stamped on the band's backdrop rect, zone label, every component shape + text label, and intra-band arrows + arrow labels. Cross-zone arrows stay ungrouped. The summary flow strip carries a single `wbgsum_*` groupId binding every dot, dot label, and connector arrow. `labelToMeta` includes `zoneGid` per component so the arrow emitter classifies intra- vs cross-band membership without a re-lookup.
